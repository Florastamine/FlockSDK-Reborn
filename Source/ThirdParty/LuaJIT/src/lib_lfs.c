...
#ifdef _WIN32
#ifdef __BORLANDC__
#define local_setmode(file, m)   (setmode(_fileno(file), m))
#define STAT_STRUCT struct stati64
#else
#define local_setmode(file, m)   (_setmode(_fileno(file), m))
...
#define _O_TEXT               0
#define _O_BINARY             0
#define local_setmode(file, m)   ((void)file, (void)m, 0)
...
#ifdef _WIN32
  #define local_mkdir _mkdir
#else
  #define local_mkdir(path) (mkdir((path), S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IXOTH))
#endif

#define LJLIB_MODULE_lfs

/*
** Utility functions
*/
static int meme(lua_State *L, const char *info)
{
    lua_pushnil(L);
    if (info == NULL)
...
LJLIB_CF(lfs_currentdir)
{
    ...
    while (1) {
        ...
        if (!path) /* failed to allocate */
            return meme(L, "get_dir realloc() failed");
        ...
        if (errno != ERANGE) { /* unexpected error */
            result = meme(L, "get_dir getcwd() failed");
            break;
        }
...
static int lfs_g_setmode(lua_State *L, FILE *f, int arg) {
    ...
    int res = local_setmode(f, mode[op]);
    if (res != -1) {
        int i;
        lua_pushboolean(L, 1);
...
{
    const char *path = luaL_checkstring(L, 1);
    return pushresult(L, local_mkdir(path), NULL);
}
...
static int lock_create_meta(lua_State *L) {
    ...
    /* Method table */
    lua_newtable(L);
    lua_pushcfunction(L, lj_cf_lfs_unlock_dir);
...
*/
LJLIB_CF(lfs_symlinkattributes)
{
    int ret;
    if (lua_isstring(L, 2) && (strcmp(lua_tostring(L, 2), "target") == 0)) {
        int ok = push_link_target(L);
        return ok ? 1 : meme(L, "could not obtain link target");
    }
...
LUALIB_API int luaopen_lfs(lua_State *L) {
    ...
    LJ_LIB_REG(L, LUA_LFSLIBNAME, lfs);
    return 1;
}
